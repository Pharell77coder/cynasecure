...
export default function MySubscriptionsPage() {
  ...
  const handleCancel = async (id: number) => {
    if (!window.confirm("Êtes-vous sûr de vouloir annuler cet abonnement ? Cette action est irréversible.")) return;
    setCancelling(id);
    try {
      await subscriptionsApi.cancel(id);
      setSubs((prev) => prev.map((s) => (s.id === id ? { ...s, status: "CANCELLED" } : s)));
      toast("Abonnement annulé avec succès", "success");
    } catch {
            ...
            {subs.map((sub) => (
              <Card
                ...
                className={`p-6 space-y-5 transition-all duration-200 ${
                  sub.status !== "ACTIVE" ? "opacity-50 grayscale" : "hover:border-blue-500/30"
                }`}
              >
                {/* Title + status */}
                ...
                {sub.status === "ACTIVE" && (
                  <div className="flex gap-3 pt-1 border-t border-gray-800">
                    <Link to={`/services/${sub.service.id}`} className="flex-1">
