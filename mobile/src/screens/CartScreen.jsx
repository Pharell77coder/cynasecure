...
const CartScreen = () => {
  ...
  const handleRemove = (item) => {
    Alert.alert('Supprimer', `Retirer "${item.name}" du panier ?`, [
      ...
      { text: 'Supprimer', style: 'destructive', onPress: () => removeFromCart(item.id, item.subscriptionType) },
    ]);
  };
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <FlatList
        data={cart}
        keyExtractor={item => `${item.id}-${item.subscriptionType}`}
          ...
          return (
            <View style={styles.card}>
              <View style={styles.iconBox}>
                <Text style={styles.iconText}>{icon(item.name)}</Text>
              </View>
              <View style={styles.info}>
                <Text style={styles.itemName} numberOfLines={2}>{item.name}</Text>
                <Text style={styles.itemSub}>
                  {item.subscriptionType === 'annual' ? 'Annuel' : 'Mensuel'} · {item.quantity} utilisateur{item.quantity > 1 ? 's' : ''}
                </Text>
                <View style={styles.qtyRow}>
                  <TouchableOpacity style={styles.qtyBtn} onPress={() => updateQuantity(item.id, item.subscriptionType, item.quantity - 1)}>
        ...
        ListFooterComponent={
          <View style={styles.summary}>
            <View style={styles.summaryLine}>
              ...
              <Text style={styles.summaryVal}>{cartTotal} € / mois</Text>
            </View>
            <View style={styles.summaryLine}>
              <Text style={styles.summaryLabel}>TVA (20%)</Text>
              <Text style={styles.summaryVal}>{Math.round(cartTotal * 0.2)} €</Text>
            </View>
            <View style={[styles.summaryLine, styles.totalLine]}>
              <Text style={styles.totalLabel}>Total TTC</Text>
              <Text style={styles.totalVal}>{Math.round(cartTotal * 1.2)} € / mois</Text>
            </View>

            {!user && (
              <View style={styles.loginHint}>
                <Text style={styles.loginHintText}>
                  💡 <Text style={styles.loginHintLink} onPress={() => navigation.navigate('Login')}>Connectez-vous</Text> pour sauvegarder votre panier.
                </Text>
              </View>
            )}

            <Button onPress={() => navigation.navigate('Checkout')} variant="primary" size="lg" fullWidth style={{ marginTop: spacing[4] }}>
              Passer à la caisse →
            </Button>
          </View>
        }
